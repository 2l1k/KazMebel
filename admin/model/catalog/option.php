...
class ModelCatalogOption extends Model {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($option_value_description['name']) . "', content = '" . $this->db->escape($option_value_content['content']) . "'");
				}

				foreach ($option_value['option_value_content'] as $language_id => $option_value_content) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_content SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', content = '" . $this->db->escape($option_value_content['content']) . "'");
				}
			}
		}
	...
	public function editOption($option_id, $data) {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				foreach ($option_value['option_value_content'] as $language_id => $option_value_content) {
					$this->db->query("UPDATE " . DB_PREFIX . "option_value_description SET content = '" . $this->db->escape($option_value_content['content']) . "' WHERE option_value_id = '" . (int)$option_value_id . "' AND language_id = '" . (int)$language_id . "' AND option_id = '" . (int)$option_id . "'");
				}
			}
	...
	public function getOptionValueDescriptions($option_id) {
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			$option_value_description_content_data = array();

			$option_value_description_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_value_description WHERE option_value_id = '" . (int)$option_value['option_value_id'] . "'");

			foreach ($option_value_description_query->rows as $option_value_description) {
				$option_value_description_content_data[$option_value_description['language_id']] = array('content' => $option_value_description['content']);
			}

			$option_value_data[] = array(
				'option_value_id'          => $option_value['option_value_id'],
				'option_value_description' => $option_value_description_data,
				'option_value_content'     => $option_value_description_content_data,
				'image'                    => $option_value['image'],
				'sort_order'               => $option_value['sort_order']
			);
		}
