...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$catname = null;
			$categ = $this->model_catalog_product->getCategories($product_info['product_id']);
			if ($categ) {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'image' => $this->model_tool_image->resize($result['image'], '1170', '525'),
					'title' => $result['title'],
					'year' => $result['year']
				);
			}
			...
			$results = $this->model_catalog_product->getProjects($this->request->get['product_id']);
			foreach ($results as $result) {
	            if ($result['image']) {
	                $image = $this->model_tool_image->resize($result['image'], '325', '165');
	            } else {
	                $image = $this->model_tool_image->resize('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
	            }
	            ...
	            $data['projects'][] = array(
	                ...
	                'title'       => $result['title'],
	                'description' => utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, $this->config->get('theme_' . $this->config->get('config_theme') . '_product_description_length')) . '..',
	                'href'        => $this->url->link('extension/d_blog_module/post', $url . '&post_id=' . $result['post_id'], 'SSL')
	            );
	        }
